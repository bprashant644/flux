...
function buildAdaptiveCard(userName, contacts) {
    ...
    attachments: [{
      ...
      content: {
        ...
        body: [
          ...
          {
            type: 'TextBlock',
            text: `You have **${contacts.length}** contact${contacts.length !== 1 ? 's' : ''} due for follow-up today.`,
            wrap: true,
            spacing: 'Small',
            isSubtle: true,
          },
...
async function postTeamsMessage(webhookUrl, userName, contacts) {
  if (!webhookUrl || !contacts.length) return;
  try {
    const res = await fetch(webhookUrl, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(buildAdaptiveCard(userName, contacts)),
    });
